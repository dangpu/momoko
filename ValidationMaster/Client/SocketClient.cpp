...
int SocketClient::read_http_header_timeout_zy(int fd, string& header, char* content, timeval *timeout)
{
    char buf[SEG_BUF + 1];
    string pkg = "";
    while(1)
    ...
    content = (char*)pkg.substr(pkg.find("\r\n\r\n")).c_str();
    printf("content: %s\n", content);
    return 0;
}
...
int SocketClient::recv_result_zy(char* result, int& len, int fd, long time_out_us)
{
    //int max_buf_len = len;
    int ret;
    len = 0;
...
bool SocketClient::getRstFromHost(const std::string& query,ServerRst& sr,unsigned short type)
{
   ...
   cerr<<"[Socket Send]:"<<sendmsg_str<<"\r\n[--Socket Send--]"<<endl;
    ...
    int rt = recv_result_zy(rcvbuf, buf_len, sock,m_timeout);
    if(rt != 0)
    {
