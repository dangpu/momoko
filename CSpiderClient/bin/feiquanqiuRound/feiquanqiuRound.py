...
import parser
import jsonlib
from feiquanqiuRoundParser import feiquanqiu_task_parser, feiquanqiu_request_parser
...
class feiquanqiuRoundParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('feiquanqiuRoundFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            try:
                ...
                logger.error('feiquanqiuRoundFlight error: Wrong Result Format %s'%str(e))
                return error
            
            if para == None:
                logger.info('feiquanqiuRoundFlight: task failed with %s for %sth time'%(content,i))
                time.sleep(random.randint(1,2))
                continue
                
            else:
                try:
                    InsertRoundFlight(para)
                    etime = time.time()#任务完成时间
                    dur = int((etime - stime) * 1000)
                    logger.info('feiquanqiuRoundFlight: task finish with %s using %d ms' %(content, dur))
                    return error
                except Exception,e:
                    logger.error('feiquanqiuRoundFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
