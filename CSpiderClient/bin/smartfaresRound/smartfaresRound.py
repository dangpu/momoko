...
import parser
import jsonlib
from smartfaresRoundParser import smartfares_task_parser, smartfares_request_parser
...
class smartfaresRoundParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('smartfaresRoundFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            result = smartfares_task_parser(content)
            ...
            try:
                ...
                logger.error('smartfaresRoundFlight error: Wrong Result Format %s'%str(e))
                return error
            
            if tickets == []:
                logger.info('smartfaresRoundFlight: task failed with %s for %sth time'%(content,i))
                ...
                try:
                    ...
                    logger.info('smartfaresRoundFlight: task finish with %s using %d ms' %(content, dur))
                    return error
                except Exception,e:
                    logger.error('smartfaresRoundFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
