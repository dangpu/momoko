...
'''
    ...
    @date:2014-04-02
    @desc:
        jijitong机票解析，将数据写入数据库
'''

import parser
import jsonlib
from jijitongParser import jijitong_task_parser
...
class jijitongParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('jijitongFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            try:
                ...
                logger.error('jijitongFlight error: Wrong Result Format %s'%str(e))
                return error
            
            if tickets == None or tickets == []:
                logger.info('jijitongFlight: task failed with %s for %sth time'%(content,i))
                ...
                try:
                    ...
                    logger.error('jijitongFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
