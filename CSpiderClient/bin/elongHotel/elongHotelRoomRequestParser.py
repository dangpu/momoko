...
def elong_room_request_parser(taskcontent):
    room_list = []
    try:
        ...
        logger.error('Cannot parse tackcontent!' + str(e))
        return room_list
...
def parseRoom(content,hotel_name,city_name_zh,check_in,check_out,hotel_id):
    ...
    try:
        ...
        logger.info('Cannot load json' + str(e))
        return room_list
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print 'Usage: python xxx.py worloadfile'
        sys.exit(-1)

    filename = sys.argv[1]
    fout = open(filename,'r')
    content_list = get_orig_data(fout)
    fout.close()

    i = 0
    for taskcontent in content_list:
        logger.info('The ' + str(i + 1) + ' taskcontent [working]')
        room_list = elong_room_parser(taskcontent)
        i += 1
