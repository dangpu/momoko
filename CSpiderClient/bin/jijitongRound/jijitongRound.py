...
import parser
import jsonlib
from jijitongRoundParser import jijitong_task_parser
#from jijitongParser import jijitong_request_parser
...
class jijitongRoundParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('jijitongRoundFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            result = jijitong_task_parser(content)
            ...
            try:
                ...
                logger.error('jijitongRoundFlight error: Wrong Result Format %s'%str(e))
                return error
            
            if para == None or para == []:
                logger.info('jijitongRoundFlight: task failed with %s for %sth time'%(content,i))
                ...
                try:
                    ...
                    logger.info('jijitongRoundFlight: task finish with %s using %d ms' %(content, dur))
                    return error
                except Exception,e:
                    logger.error('jijitongRoundFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
