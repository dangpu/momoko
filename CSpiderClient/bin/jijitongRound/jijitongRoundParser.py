...
FIRST_URL = 'http://www.jijitong.com/round-%s-%s.html?%s-%s'
HOST = 'http://www.jijitong.com'
...
def jijitong_task_parser(taskcontent):
    result = {}
    result['para'] = {'flight':{},'ticket':[]}
    result['error'] = 0

    taskcontent.encode('utf-8')
    try:
        dept_city_zh,dept_city_en,dest_city_zh,dest_city_en,dept_day_temp,dest_day_temp = \
                ...
                taskcontent.strip().split('&')[5]
        dept_day = dept_day_temp[0:4] + '-' + dept_day_temp[4:6] + '-' + dept_day_temp[6:8]
        dest_day = dest_day_temp[0:4] + '-' + dest_day_temp[4:6] + '-' + dest_day_temp[6:8]
    except Exception,e:
...
def get_post_data(content, dept_day, dest_day):
    post_data = {}
    try:
    ...
    for each_flight_json in flight_json['datalist']:
        ...
        #start
        value += '<item><key>'
        value += each_flight_json['Key'] + '</key>'
        ...
        if (BackAirCode == '' and goAirCode == BackAirCode) or (BackAirCode != '' and goAirCode != ''\
...
if __name__ == '__main__':
    ...
    taskcontent = '广州&guangzhou&罗马&luoma&20140703&20140801'
