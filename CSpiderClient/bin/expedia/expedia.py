...
'''
    ...
    @date:2014-06-27
    @desc:
        expediaFlight机票解析，将数据写入数据库
'''

import parser
import jsonlib
from expediaParser import expedia_task_parser
from expediaParser import expedia_request_parser
...
class expediaParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('expediaFlightFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            try:
                ...
                logger.error('expediaFlight error: Wrong Result Format %s'%str(e))
                return error
            ...
            if tickets == []:
                ...
                try:
                    ...
                    logger.info('expediaFlight: task finish with %s using %d ms' %(content, dur))
                    return error
                except Exception,e:
                    logger.error('expediaFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
