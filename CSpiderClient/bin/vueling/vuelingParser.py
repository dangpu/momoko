...
def vueling_request_parser(content):
    ...
    try:
        ...
        logger.error('vuelingFlight: Wrong Content Format with %s'%content)
        return result

    url = 'http://tickets.vueling.com/ScheduleSelect.aspx'
    Referer = 'http://tickets.vueling.com/ScheduleSelect.aspx'

    content = request_post_data(url,postdata,referer=Referer,proxy=None,\
        Accept="text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8")

    if content == '' or content == None:
        return result

    result = vuelingvalidate(content,flight_no,req_dept_time)
    
    return result


def vuelingparser(content):
    allinfos = []
    #get flight num
    flight_num_list = []
