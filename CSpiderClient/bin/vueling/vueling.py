...
'''
    ...
    @date:2014-04-02
    @desc:
        vueling机票解析，将数据写入数据库
'''

import parser
import jsonlib
from vuelingParser import vueling_task_parser
from vuelingParser import vueling_request_parser
...
class vuelingParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('vuelingFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            result = vueling_task_parser(content)

            try:
                para = result['para']
                error = result['error']
            except Exception,e:
                logger.error('vuelingFlight error: Wrong Result Format %s'%str(e))
                return error
            
            if para == None or para == []:
                logger.info('vuelingFlight: task failed with %s for %sth time'%(content,i))
                ...
                try:
                    ...
                    logger.error('vuelingFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
