...
'''
    ...
    @date:2014-04-02
    @desc:
        youzhan hotel&room解析，将数据写入数据库
'''

import parser
from youzhanParser import youzhan_task_parser
from youzhanParser import youzhan_request_parser
...
class youzhanParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info ('youzhanHotel: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            if para == None or para == []:
                logger.error ('youzhanHotel: task failed with %s for %sth time'%(content,i))
                ...
                try:
                    ...
                    logger.info("youzhanHotel: task %s finish using %d ms"%(content, dur))
                    return error
                except Exception,e:
                    logger.error('youzhanHotel: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error         
