...
import parser
import jsonlib
from feifanParser import feifan_task_parser
from feifanParser import feifan_request_parser
...
class feifanParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('feifanFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            result = feifan_task_parser(content)

            try:
                para = result['para']
                error = result['error']
            except Exception,e:
                logger.error('feifanFlight error: Wrong Result Format %s'%str(e))
                return error
            
            if para == None or para == []:
                logger.info('feifanFlight: task failed with %s for %sth time'%(content,i))
                ...
                try:
                    ...
                    logger.error('feifanFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
