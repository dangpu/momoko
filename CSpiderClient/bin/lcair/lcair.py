...
'''
    ...
    @date:2014-06-27
    @desc:
        lcairFlight机票解析，将数据写入数据库
'''

import parser
import jsonlib
from lcairParser import lcair_task_parser
from lcairParser import lcair_request_parser
...
class lcairParser(parser.Parser):
    def __init__(self):
        pass
    ...
    def parse(self,task):
        ...
        logger.info('lcairFlightFlight: start a new task @ %s'%str(update_time))
        ...
        for i in range(2):
            ...
            try:
                ...
                logger.error('lcairFlight error: Wrong Result Format %s'%str(e))
                return error
            ...
            if tickets == []:
                ...
                try:
                    ...
                    logger.info('lcairFlight: task finish with %s using %d ms' %(content, dur))
                    return error
                except Exception,e:
                    logger.error('lcairFlight: Insertation Error: %s'%str(e))
                    error = DB_ERR_CODE
                    return error
