...
class SocketClient
{
	...
	private:
		...
		int read_http_header_timeout_zy(int fd, std::string& header, std::string& content, timeval *timeout);
		int recv_result(char* result, int& len, int fd,long time_out_us);
		int recv_result_zy(std::string& result, int& len, int fd,long time_out_us);
